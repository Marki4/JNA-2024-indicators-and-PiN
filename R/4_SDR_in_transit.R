...
pins_indicator_other_n <- main_mmm %>%
  select(c(B06_travel_with_children, 
           B13_pregnant, 
           B14_lactating, 
           #Education
...
weighted_pins_indicator_other_n <- pins_indicator_other_n %>%
  mutate(across(c(IND_FS,
                  IND_HE,
                  IND_HTS,
                  IND_NUT_T1,
...
ds_per_sector_other_n <- weighted_pins_indicator_other_n %>%
  mutate(
    Food_security_ds = IND_FS,
    Health_ds =  IND_HE,
    Integration_ds = rowSums(weighted_pins_indicator_other_n[, c("IND_INT_T1")], na.rm = TRUE),
    Nutrition_ds = rowSums(weighted_pins_indicator_other_n[, c("IND_NUT_T1", "IND_NUT_T2", "IND_NUT_T3" )], na.rm = TRUE),
    Nutrition_ds_women = rowSums(weighted_pins_indicator_other_n[, c("IND_NUT_T1" )], na.rm = TRUE),
    Nutrition_ds_baby = rowSums(weighted_pins_indicator_other_n[, c("IND_NUT_T2", "IND_NUT_T3" )], na.rm = TRUE),
    Protection_ds = rowSums(weighted_pins_indicator_other_n[, c("IND_PRO_T1", "IND_PRO_T2", "IND_PRO_CP", "IND_PRO_GBV", "IND_HTS")], na.rm = TRUE),
    Child_protection_ds = IND_PRO_CP,
    Gender_based_violence_ds = IND_PRO_GBV,
    HT_S_ds = IND_HTS,
    Shelter_ds = rowSums(weighted_pins_indicator_other_n[, c("IND_SHE_T1", "IND_SHE_T2")], na.rm = TRUE),
    Wash_ds = rowSums(weighted_pins_indicator_other_n[, c("IND_WA_T1", "IND_WA_T2", "IND_WA_T3" )], na.rm = TRUE),
    Education_ds = rowSums(weighted_pins_indicator_other_n[, c("IND_EDU_T1")], na.rm = TRUE)
  )%>%
  rename()
...
df_pin_other_n <- ds_per_sector_other_n %>%
  mutate(intersector_ds = rowSums(select(., c(
    "Food_security_ds",
    "Health_ds",
    "Integration_ds",
...
ggplot(df_pin_other_n, aes(x = intersector_ds)) +
  geom_density(fill = "blue", alpha = 0.9) +
  labs(title = "Density Plot of Intersector IPM Distribution",
       x = "Intersector ds",
       y = "Density") +
...
pin_intersector_other_n <- sum(df_pin_other_n$"intersector_pin") / nrow(df_pin_other_n)
...
df_sectoral_pin_other_n <- df_pin_other_n %>%
  select(-intersector_ds) %>%
  #include in sectorial pins if included in intersectorial pin (intersector== 1) 
  #and there is a privation in the specific sector (sector_ds>0)
  mutate(across(
...
df_sectoral_pin_edu_other_n <- df_pin_other_n %>%
  filter(B06_travel_with_children=="yes")%>%
  select(-intersector_ds, Education_ds) %>%
  #include in sectorial pins if included in intersectorial pin (intersector== 1) 
  #and there is a privation in the specific sector (sector_ds>0)
...
#final Education pin
pins_final_edu_other_n <- data.frame(  
  Sectors = c(
    "Education"
  ),
  pins = round(colSums(select(df_sectoral_pin_edu_other_n, Education_pin)) / nrow(df_sectoral_pin_edu_other_n) *
                 100, 1)
)

#Nutrition women
df_sectoral_pin_nut_w_other_n <- df_pin_other_n %>%
  filter(B13_pregnant=="yes"| B14_lactating=="yes")%>%
  select(-intersector_ds, Nutrition_ds_women) %>%
  #include in sectorial pins if included in intersectorial pin (intersector== 1) 
  #and there is a privation in the specific sector (sector_ds>0)
...
#final Nutrition women pin
pins_final_nut_w_other_n <- data.frame(  
  Sectors = c(
    "Nutrition Woman"
  ),
  pins = round(colSums(select(df_sectoral_pin_nut_w_other_n, Nutrition_pin_women)) / nrow(df_sectoral_pin_nut_w_other_n) *
                 100, 1)
)


#Nutrition  households with children
df_sectoral_pin_nut_b_other_n <- df_pin_other_n %>%
  filter(B06_travel_with_children=="yes")%>%
  select(-intersector_ds, Nutrition_ds_baby) %>%
  #include in sectorial pins if included in intersectorial pin (intersector== 1) 
  #and there is a privation in the specific sector (sector_ds>0)
...
#final Nutrition baby pin
pins_final_nut_b_other_n <- data.frame(  
  Sectors = c(
    "Nutrition Baby"
  ),
  pins = round(colSums(select(df_sectoral_pin_nut_b_other_n, Nutrition_pin_baby)) / nrow(df_sectoral_pin_nut_b_other_n) *
                 100, 1)
)

#Child Protection
df_sectoral_pin_cp_other_n <- df_sectoral_pin_other_n%>%
  filter(B06_travel_with_children=="yes")%>%
  #include in sectorial pins if included in intersectorial pin (intersector== 1) 
  #and there is a privation in the specific sector (sector_ds>0)
  mutate(across(
...
#final child protection
pins_final_cp_other_n <- data.frame(  
  Sectors = c(
    "Child Protection"
  ),
  pins = round(colSums(select(df_sectoral_pin_cp_other_n, Child_protection_pin)) / nrow(df_sectoral_pin_cp_other_n) *
                 100, 1)
)


#final PIN table
pins_final_other_n <- data.frame(  
  Sectors = c(
    "Food_security",
    "Health",
    "Integration",
  ...
  pins = round(colSums(select(df_sectoral_pin_other_n, Food_security_pin, Health_pin, 
                              Integration_pin,
                              Protection_pin, Gender_based_violence_pin,
                              HT_S_pin,
                              Shelter_pin, Wash_pin, intersector_pin)) / nrow(df_sectoral_pin_other_n) *
                 100, 1)
)
...
pins_final_final_other_n <- rbind(pins_final_other_n, pins_final_edu_other_n, pins_final_nut_w_other_n, pins_final_nut_b_other_n, pins_final_cp_other_n)

write_xlsx(pins_final_final_other_n, "pins_final_final_other_n.xlsx")
